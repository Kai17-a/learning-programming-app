...
async fn test_complete_file_change_to_execution_workflow() -> Result<()> {
    ...
    assert_eq!(
        ...
        "History record should show success"
    );
    ...
    assert_eq!(
        ...
        "Successful executions should be 1"
    );
...
async fn test_file_watching_and_auto_execution() -> Result<()> {
    ...
    assert!(
        file_change_received.is_ok(),
        "File change should be detected within timeout"
    );
    assert!(
        file_change_received.unwrap(),
        "File change event should be received"
    );
    ...
    assert!(
        result.output.contains("Auto execution test"),
        "Output should contain expected text"
    );
...
async fn test_multiple_executions_and_history() -> Result<()> {
    ...
    for (filename, content) in files {
        ...
        assert!(
            result.success,
            "Execution of {} should be successful",
            filename
        );
    }
    ...
    for record in &history {
        ...
        assert!(
            !record.output_preview.is_empty(),
            "Output preview should not be empty"
        );
    }
...
async fn test_syntax_error_handling() -> Result<()> {
    ...
    assert!(
        error_msg.contains("SyntaxError") || error_msg.contains("syntax"),
    ...
    assert_eq!(
        ...
        "History record should show failure"
    );
    ...
    assert_eq!(
        ...
        "Successful executions should be 0"
    );
...
async fn test_system_health_and_status() -> Result<()> {
    ...
    assert!(
        !status.registered_handlers.is_empty(),
...
async fn test_concurrent_executions() -> Result<()> {
    ...
    for (i, result) in results.into_iter().enumerate() {
        ...
        assert!(
            ...
            "Concurrent execution {} should be successful",
            i
        );
        assert!(
            execution_result
                .output
                .contains(&format!("Concurrent execution {}", i)),
            "Output should contain expected text for execution {}",
            i
        );
    }
...
func main() {
    ...
    assert!(
        ...
        "File changes should be detected within timeout"
    );
    assert_eq!(
        detected_files.len(),
        4,
        "All file changes should be detected"
    );
    ...
    assert!(
        py_result.success,
        "Python file execution should be successful"
    );
...
"#,
    ...
    assert!(
        result.execution_time.as_millis() < u128::MAX,
        "Execution time should be recorded"
    );
    ...
    if result.success {
        assert!(
            result.output.contains("Hello, World from Go!"),
            "Output should contain expected text"
        );
    }
...
mod test_helpers {
    ...
    #[allow(dead_code)]
    pub async fn create_section_with_files(
        base_dir: &Path,
        section_name: &str,
        files: Vec<(&str, &str)>,
