...
async fn test_large_file_monitoring_performance() -> Result<()> {
    ...
    assert!(
        watch_setup_time < Duration::from_secs(5),
        "File watcher setup should be fast"
    );
    ...
    assert!(
        change_detected.is_ok() && change_detected.unwrap(),
...
async fn test_concurrent_execution_performance() -> Result<()> {
    ...
    println!(
        "Completed {} concurrent executions in {:?}",
    ...
    for (i, result) in results.into_iter().enumerate() {
        ...
        println!(
            "Concurrent task {} completed in {:?} (success: {})",
            i, individual_time, success
        );
    }

    assert_eq!(
        successful_count, num_concurrent,
...
async fn test_database_performance_under_load() -> Result<()> {
    ...
    println!(
        "Completed {} database operations in {:?}",
        num_executions, db_total_time
    );
    ...
    assert_eq!(
        history.len(),
        num_executions,
        "All executions should be in history"
    );
    assert!(
        query_time < Duration::from_secs(2),
        "History query should be fast"
    );
    ...
    assert_eq!(
        stats.total_executions, num_executions as u64,
        "Statistics should be accurate"
    );
    assert!(
        stats_time < Duration::from_secs(1),
...
async fn test_memory_and_resource_management() -> Result<()> {
    ...
    for round in 0..10 {
        for (filename, content) in &test_cases {
            ...
            assert!(
                !result.output.is_empty() || result.error_message.is_some(),
    ...
    assert!(
        final_time < Duration::from_secs(5),
        "Final execution should be fast"
    );
...
async fn test_system_stability_under_load() -> Result<()> {
    ...
    while stability_start.elapsed() < test_duration {
        for (filename, _, expected_success) in &test_files {
            ...
            assert_eq!(
                ...
                "Execution result should be consistent for {}",
                filename
            );
    ...
    println!(
        "Stability test completed: {} executions in {:?}",
        execution_count, total_time
    );
    println!(
        "Success rate: {:.2}%",
    ...
    println!(
        "Average execution time during stability test: {:?}",
        avg_execution_time
    );
    ...
    let stats = app_service.history.get_stats().await?;
    assert_eq!(
        stats.total_executions, execution_count as u64,
        "Statistics should be accurate"
    );
    assert_eq!(
        stats.successful_executions, success_count as u64,
...
async fn test_file_watcher_responsiveness() -> Result<()> {
    ...
    for i in 0..num_changes {
        ...
        assert!(
            detected.is_ok() && detected.unwrap(),
    ...
    assert!(
        avg_detection_time < Duration::from_millis(500),
