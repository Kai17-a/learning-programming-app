...
async fn test_error_recovery_and_continuation() -> Result<()> {
    ...
    for (filename, content, expected_success) in test_sequence {
        ...
        assert_eq!(
            ...
            "Execution of {} should have success={}",
    ...
    assert_eq!(
        ...
        6,
        "All files should have been executed"
    );
    ...
    assert_eq!(
        history.len(),
        6,
        "History should contain all execution attempts"
    );
    ...
    assert_eq!(
        ...
        "Should have 3 successful executions"
    );
    assert_eq!(
        stats.failed_executions, 3,
        "Should have 3 failed executions"
    );
...
async fn test_file_watcher_error_continuation() -> Result<()> {
    ...
    assert!(
        detected_changes.len() >= 3,
        "File watcher should continue detecting changes after errors"
    );
    ...
    for (filename, _content, expected_success) in test_files {
        ...
        assert_eq!(
            result.success, expected_success,
            "Execution of {} should match expected result",
            filename
        );
    }
...
async fn test_concurrent_error_recovery() -> Result<()> {
    ...
    for (i, result) in results.into_iter().enumerate() {
        ...
        assert!(
            ...
            "Error execution {} should fail",
            i
        );
        assert!(
            execution_result.error_message.is_some(),
            "Error message should be present for execution {}",
            i
        );
    }
    ...
    assert_eq!(
        history.len(),
        6,
        "History should contain all 6 executions (5 errors + 1 success)"
    );
    ...
    assert!(
        status.is_healthy(),
        "System should be healthy after error recovery"
    );
...
async fn test_file_system_error_handling() -> Result<()> {
    ...
    assert!(
        !result.success,
        "Execution of non-existent file should fail"
    );
    assert!(
    ...
    assert!(
        recovery_result.success,
        "System should recover from file system errors"
    );
...
async fn test_resource_management_under_errors() -> Result<()> {
    ...
    assert!(
        ...
        "System should remain responsive after many errors"
    );
    ...
    let stats = app_service.history.get_stats().await?;
    assert_eq!(
        stats.total_executions, 21,
        "Should have 21 total executions"
    );
    assert_eq!(
    ...
    assert_eq!(
        stats.failed_executions, 20,
        "Should have 20 failed executions"
    );
...
async fn test_file_watcher_startup_error_handling() -> Result<()> {
    ...
    let watch_result = app_service
        .watcher
        .start_watching(&nonexistent_dir, tx)
        .await;

    // Verify error is handled gracefully
    assert!(
        watch_result.is_err(),
        "Watching non-existent directory should fail"
    );

    // Verify watcher is not in an inconsistent state
    assert!(
        !app_service.watcher.is_watching(),
        "Watcher should not be active after error"
    );
    ...
    assert!(
        valid_watch_result.is_ok(),
        "Should be able to watch valid directory after error"
    );
    assert!(
        app_service.watcher.is_watching(),
        "Watcher should be active for valid directory"
    );
