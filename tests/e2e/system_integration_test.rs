...
async fn test_complete_system_integration() -> Result<()> {
    ...
    // Create sample files
    fs::write(section1_dir.join("hello_world.py"), "print('Hello, World!')").await?;
    fs::write(section1_dir.join("variables.py"), "name = 'Python'\nprint(f'Hello, {name}!')").await?;
    fs::write(section2_dir.join("if_statements.py"), "x = 10\nif x > 5:\n    print('x is greater than 5')").await?;
    fs::write(section2_dir.join("loops.py"), "for i in range(3):\n    print(f'Loop iteration: {i}')").await?;
    ...
    assert!(sections.contains(&"section1-basics".to_string()), "Should contain section1-basics");
    assert!(sections.contains(&"section2-control-flow".to_string()), "Should contain section2-control-flow");
...
async fn test_cli_file_watching_integration() -> Result<()> {
    ...
    assert!(watched_dirs.is_empty(), "No directories should be watched initially");
...
async fn test_application_service_component_integration() -> Result<()> {
    ...
    let supported_extensions = app_service.language_handlers.get_supported_extensions().await;
    assert!(!supported_extensions.is_empty(), "Should have supported extensions");
    assert!(supported_extensions.contains(&"py".to_string()), "Should support Python");
    ...
    assert!(result.output.contains("Integration test successful"), "Output should contain expected text");
    ...
    assert!(file_change_received.is_ok() && file_change_received.unwrap(), 
    ...
    assert!(watch_result.success, "Watched file execution should be successful");
...
async fn test_error_propagation_through_layers() -> Result<()> {
    ...
    assert!(!syntax_result.success, "Syntax error should propagate as failure");
    assert!(syntax_result.error_message.is_some(), "Error message should be propagated");
    ...
    assert!(!runtime_result.success, "Runtime error should propagate as failure");
    assert!(runtime_result.error_message.is_some(), "Runtime error message should be propagated");
    ...
    assert!(!history[0].success, "First error should be recorded as failure");
    assert!(!history[1].success, "Second error should be recorded as failure");
    ...
    assert!(success_result.success, "System should continue working after errors");
...
async fn test_concurrent_resource_access() -> Result<()> {
    ...
    for (i, result) in results.into_iter().enumerate() {
        ...
        assert!(history_len > 0, "History should be accessible during concurrent execution");
    }
    
    // Verify final state
    let final_history = app_service.history.get_history(None).await?;
    assert_eq!(final_history.len(), 10, "All concurrent executions should be recorded");
    
    let stats = app_service.history.get_stats().await?;
    assert_eq!(stats.total_executions, 10, "Statistics should reflect all executions");
    assert_eq!(stats.successful_executions, 10, "All concurrent executions should be successful");
...
async fn test_system_under_load() -> Result<()> {
    ...
    assert_eq!(successful_count, num_files, "All load test executions should be successful");
    
    // Verify system performance (should complete within reasonable time)
    assert!(execution_time.as_secs() < 30, "Load test should complete within 30 seconds");
    
    // Verify database integrity after load
    let history = app_service.history.get_history(None).await?;
    assert_eq!(history.len(), num_files, "All executions should be recorded in history");
    
    let stats = app_service.history.get_stats().await?;
    assert_eq!(stats.total_executions, num_files as u64, "Statistics should be accurate");
    
    // Verify system is still healthy after load
    assert!(app_service.history.is_healthy().await, "Database should remain healthy after load");
    
    let status = app_service.get_system_status().await;
    assert!(status.is_healthy(), "System should remain healthy after load test");
...
async fn test_system_lifecycle() -> Result<()> {
    ...
    for cycle in 0..3 {
        ...
        let status = app_service.get_system_status().await;
        assert!(status.is_healthy(), "System should be healthy after initialization (cycle {})", cycle);
        ...
        fs::write(&test_file, format!("print('Lifecycle test cycle {}')", cycle)).await?;
        
        let result = app_service.executor.execute_file(&test_file).await;
        assert!(result.success, "Execution should be successful in cycle {}", cycle);
        
        // Verify operations were recorded
        let history = app_service.history.get_history(None).await?;
        assert_eq!(history.len(), 1, "History should contain one record in cycle {}", cycle);
