...
impl HistoryManagerService {
    ...
    pub async fn new(db_path: impl AsRef<Path>) -> Result<Self> {
        ...
        info!(
            "HistoryManagerService initialized with database: {}",
            db_url
        );
        Ok(service)
    }
    ...
    pub async fn get_history(&self, limit: Option<u32>) -> Result<Vec<ExecutionRecord>> {
        ...
        let records: Vec<ExecutionRecord> = rows
            .into_iter()
    ...
    pub async fn get_stats(&self) -> Result<ExecutionStats> {
        ...
        let success_row = sqlx::query(
            "SELECT COUNT(*) as successful FROM execution_history WHERE success = true",
        )
        .fetch_one(&self.db_pool)
        .await?;
        ...
        let avg_time_row =
            sqlx::query("SELECT AVG(execution_time) as avg_time FROM execution_history")
                .fetch_one(&self.db_pool)
                .await?;

        let average_execution_time = avg_time_row
        ...
        let last_execution_row =
            sqlx::query("SELECT MAX(timestamp) as last_timestamp FROM execution_history")
                .fetch_one(&self.db_pool)
                .await?;
    ...
    pub async fn get_file_stats(&self, file_path: impl AsRef<str>) -> Result<ExecutionStats> {
        ...
        let total_row =
            sqlx::query("SELECT COUNT(*) as total FROM execution_history WHERE file_path = ?")
                .bind(file_path)
                .fetch_one(&self.db_pool)
                .await?;
        ...
        let avg_time_row = sqlx::query(
            "SELECT AVG(execution_time) as avg_time FROM execution_history WHERE file_path = ?",
        )
        .bind(file_path)
        .fetch_one(&self.db_pool)
        .await?;
        ...
        let last_execution_row = sqlx::query(
            "SELECT MAX(timestamp) as last_timestamp FROM execution_history WHERE file_path = ?",
        )
        .bind(file_path)
        .fetch_one(&self.db_pool)
        .await?;
    ...
    pub async fn get_section_stats(&self, section: impl AsRef<str>) -> Result<ExecutionStats> {
        ...
        let total_row =
            sqlx::query("SELECT COUNT(*) as total FROM execution_history WHERE section = ?")
                .bind(section)
                .fetch_one(&self.db_pool)
                .await?;
        ...
        let avg_time_row = sqlx::query(
            "SELECT AVG(execution_time) as avg_time FROM execution_history WHERE section = ?",
        )
        .bind(section)
        .fetch_one(&self.db_pool)
        .await?;
        ...
        let last_execution_row = sqlx::query(
            "SELECT MAX(timestamp) as last_timestamp FROM execution_history WHERE section = ?",
        )
        .bind(section)
        .fetch_one(&self.db_pool)
        .await?;
