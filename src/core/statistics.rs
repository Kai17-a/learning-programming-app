...
impl StatisticsService {
    ...
    pub async fn display_overview(&self) -> Result<()> {
        ...
        if let Some(most_executed) = &stats.most_executed_file {
            term.write_line(&format!("Most executed: {}", style(most_executed).cyan()))?;
        }
    ...
    pub async fn display_file_stats(&self, file_path: impl AsRef<str>) -> Result<()> {
        ...
        let term = Term::stdout();
        term.write_line(&format!("📄 {} Statistics", style(file_path).cyan().bold()))?;
        term.write_line(&style("─".repeat(50)).dim().to_string())?;

        if stats.total_executions == 0 {
            term.write_line(
                &style("No execution history found for this file.")
                    .dim()
                    .to_string(),
            )?;
            return Ok(());
    ...
    pub async fn display_section_stats(&self, section: impl AsRef<str>) -> Result<()> {
        ...
        if stats.total_executions == 0 {
            term.write_line(
                &style("No execution history found for this section.")
                    .dim()
                    .to_string(),
            )?;
            return Ok(());
