...
use crate::core::models::{ExecutionRecord, ExecutionResult};
...
impl AutoExecutorService {
    ...
    pub async fn execute_file(&self, file_path: impl AsRef<Path>) -> ExecutionResult {
        ...
        if !path.exists() {
            ...
            println!("{}", error_msg);
            return ExecutionResult::new(path.to_path_buf()).with_error(
        ...
        let extension = match path.extension().and_then(|ext| ext.to_str()) {
            ...
            None => {
                ...
                let error_msg = self
                    .error_handler
                    .handle_execution_error(&app_error.into(), path);
                println!("{}", error_msg);
                return ExecutionResult::new(path.to_path_buf()).with_error(
        ...
        let handler = match self.language_handlers.get_handler(extension).await {
            ...
            None => {
                let app_error =
                    AppError::execution(format!("No handler found for extension: .{}", extension));
                let error_msg = self
                    .error_handler
                    .handle_execution_error(&app_error.into(), path);
                println!("{}", error_msg);
        ...
        let result = match handler.execute(path).await {
            Ok(result) => {
                if result.success {
                    ...
                    if error_msg.contains("SyntaxError") || error_msg.contains("syntax") {
                        let formatted_error =
                            self.error_handler.handle_syntax_error(error_msg, path);
                        println!("{}", formatted_error);
                    } else {
                        let formatted_error =
                            self.error_handler.handle_runtime_error(error_msg, path);
                        println!("{}", formatted_error);
                    }
                }
                result
            ...
            Err(e) => {
                ...
                println!("{}", error_msg);
                ExecutionResult::new(path.to_path_buf()).with_error(
    ...
    #[allow(dead_code)]
    pub async fn execute_file_with_timeout(
        &self,
        ...
        .await
        {
            ...
            Err(_) => {
                ...
                println!("{}", error_msg);
                ExecutionResult::new(path).with_error(
                    format!("Execution timeout ({}s)", timeout_secs),
                    std::time::Duration::from_secs(timeout_secs),
                    None,
                )
            }
...
mod tests {
    ...
    async fn test_format_output_success() {
        ...
        let result = ExecutionResult::new(std::path::PathBuf::from("test.py")).with_success(
    ...
    async fn test_format_output_failure() {
        ...
        let result = ExecutionResult::new(std::path::PathBuf::from("test.py")).with_error(
