...
impl DisplayService {
    ...
    pub fn display_execution_result(&self, result: &ExecutionResult) -> std::io::Result<()> {
        ...
        if result.success {
            if !result.output.is_empty() {
                ...
                self.term
                    .write_line(&format!("  {}", style("(no output)").dim()))?;
            }
        } else {
            // Print error message
            if let Some(error_msg) = &result.error_message {
                ...
                self.term
                    .write_line(&format!("  {}", style("Unknown error").red()))?;
            }
    ...
    pub fn display_startup(
        ...
        for line in banner {
            self.term
                .write_line(&style(line).cyan().bold().to_string())?;
        }
        ...
        for instruction in instructions {
            self.term
                .write_line(&style(instruction).dim().to_string())?;
        }

        self.term.write_line("")?;
        self.term
            .write_line(&style("Waiting for file changes...").green().to_string())?;
        self.term
            .write_line(&style("═".repeat(60)).dim().to_string())?;
        self.term.write_line("")?;

        info!(
            "Application started, watching directory: {}",
            watch_dir.display()
        );
    ...
    pub fn display_stats(&self, stats: &ExecutionStats) -> std::io::Result<()> {
        self.term
            .write_line(&style("📊 Execution Statistics").blue().bold().to_string())?;
        self.term
            .write_line(&style("─".repeat(30)).dim().to_string())?;
        ...
        if let Some(most_executed) = &stats.most_executed_file {
            self.term
                .write_line(&format!("Most executed: {}", style(most_executed).cyan()))?;
        }
    ...
    pub fn display_info(&self, message: &str) -> std::io::Result<()> {
        let formatted_message = format!("{} {}", style("ℹ️  Info:").blue().bold(), message);
...
mod tests {
    ...
    fn test_log_execution_result() {
        ...
        let success_result = ExecutionResult::new(PathBuf::from("test.py")).with_success(
            "Hello".to_string(),
        ...
        let error_result = ExecutionResult::new(PathBuf::from("test.py")).with_error(
